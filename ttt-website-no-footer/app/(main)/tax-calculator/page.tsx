              ...
              <div className="bg-slate-100 p-1 rounded-xl flex mb-8">
                {(["monthly", "yearly"] as const).map((p) => (
                  <button
                    key={p}
                    onClick={() => setPeriod(p)}
                    className={`flex-1 py-3 sm:py-2 text-sm font-semibold rounded-lg transition-all capitalize ${period === p
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#0077BB] to-[#01527e] rounded-2xl shadow-xl text-white p-5 sm:p-8">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 border-t border-white/10 pt-6">
                <div>
                  <p className="text-blue-200 text-xs sm:text-sm mb-1">Annual Net Pay</p>
                  <p className="text-lg sm:text-xl font-semibold">R {fmt(results.annualNetPay)}</p>
                </div>
                <div>
                  <p className="text-blue-200 text-xs sm:text-sm mb-1">Total Tax (PAYE)</p>
                  <p className="text-lg sm:text-xl font-semibold">R {fmt(results.finalTaxPayable)}</p>
                </div>
              </div>
            </div>
...
function Row({
  ...
  return (
    <div
      className={`flex justify-between gap-3 text-sm ${green
        ? "text-emerald-600"
        : accent
